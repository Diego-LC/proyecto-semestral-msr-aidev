#!/usr/bin/env python3
"""Create the reproducible sample for the merged-after-rework AIDev flow."""
...
def summarize_result(
    ...
    population_filter_counts: Dict[str, int],
) -> Dict:
    return {
        "seed": result.seed,
        ...
        "population_filter_counts": population_filter_counts,
    }
...
def main() -> None:
    ...
    population_filter_counts = summarize_population_filters(source_rows)
    ...
    summary = summarize_result(population_rows, result, population_filter_counts)
    ...
    print(
        json.dumps(
            {"output_csv": str(args.output_csv), "summary_json": str(args.summary_json)},
            ensure_ascii=False,
        )
    )
